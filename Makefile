...
nitro-root-cert:
	...
	unzip -p $(AWS_NITRO_ROOT_CERT_DIR)/AWS_NitroEnclaves_Root-G1.zip AWS_NitroEnclaves_Root-G1.pem > $(AWS_NITRO_ROOT_CERT)
...
enclave-eif-build:
	nitro-cli build-enclave --docker-uri $(ENCLAVE_IMAGE) --output-file $(ENCLAVE_EIF)
